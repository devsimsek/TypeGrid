...
### Fixed
- **Frontend Markdown Parser**: Overhauled the frontend markdown rendering engine to correctly isolate block elements. Headers, lists, code blocks, and blockquotes will no longer be erroneously wrapped inside or merged with adjacent `<p>` tags. 
- Added support for standard Markdown hard line breaks (leaving two spaces at the end of a line now correctly outputs a `<br />`).

## [4.5.1] - 2026-04-14

### Fixed
- **Navigation Visibility**: The "Posts" and "Pages" navigation menu items are now completely hidden (including their list items) if no posts or pages exist.
...
### Added
- **Manual Sitemap Generation**: Added a dedicated `Generate Sitemap` option to the CLI main menu (`npm run typegrid`) to manually build `sitemap.xml` without running the full API generator.
